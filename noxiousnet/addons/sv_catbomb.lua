...
local function DoCatBomb(pl)
	...
	timer.CreateEx("catbombmessage", 1, 5, PrintMessage, HUD_PRINTCENTER, "INCOMING CATBOMB!")
	
	local tr = util.TraceLine({start=pl:EyePos(), endpos = pl:EyePos() + pl:GetAimVector() * 10000, ignore=pl})
	...
	if ent:IsValid() then
		...
		timer.SimpleEx(8.5, CatBombExplode, ent)
		CATBOMB_ENTITY = ent
	end
end
...
function CatBombExplode(prop)
	...
	for i=1, 30 do
		timer.SimpleEx(i*0.35, BroadcastLua, [[surface.PlaySound("ambient/explosions/explode_"..math.random(8)..".wav")]])
	end
	timer.CreateEx("CATBOMBFUCKEVERYTHING", 0.75, 15, KillEverything)
	for i=1, 65 do
